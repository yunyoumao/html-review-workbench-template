...
TEXT_EXTENSIONS = {".md", ".py", ".html", ".js", ".json", ".txt", ".yml", ".yaml"}
...
def validate_json() -> list[str]:
    errors: list[str] = []
    data = json.loads((ROOT / "data" / "sample-review.json").read_text(encoding="utf-8"))
    if not data.get("items"):
        errors.append("sample-review.json has no items.")
    ids = [item.get("id") for item in data.get("items", [])]
    if len(ids) != len(set(ids)):
        errors.append("sample-review.json has duplicate ids.")
    return errors
